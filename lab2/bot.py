...
async def check_with_ai(content: str) -> str:
    ...
    prompt = f"""
Ты — строгий преподаватель программирования. Проанализируй лабораторную работу и дай четкую оценку.

РАБОТА СТУДЕНТА:
{content}

КРИТЕРИИ (100 баллов):
1. Качество кода (0-30) - читаемость, архитектура, эффективность
2. Полнота реализации (0-30) - выполнение всех требований задания  
3. Документация (0-20) - комментарии, описание, ясность
4. Оформление (0-20) - структура, форматирование, чистота

ФОРМАТ ОТВЕТА (будь краток и конкретен):
• [1-2 предложения] Суть работы
• [по 1-2 пункта на критерий] Сильные и слабые стороны
• [конкретные пункты] Что исправить
• [число] Итоговая оценка

Без лишних слов - только по делу. Будь строг, но справедлив.
"""
